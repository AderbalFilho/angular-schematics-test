...
import { OtherOptions } from './schema';
...
function filterTemplates(options: OtherOptions): Rule {
  if (!options.otherService) {
    return filter(path => !path.match(/\.service\.ts$/) && !path.match(/-item\.ts$/) && !path.match(/\.bak$/));
  }
  return filter(path => !path.match(/\.bak$/));
}

export default function (options: OtherOptions): Rule {
